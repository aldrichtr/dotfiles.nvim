...
function fs.directoryname(path)
...
--- @param matches table List of 0 or more regex to apply to the `name`
--- @param excludes table List of 0 or more regex to apply to name and path
...
---@field matches? table A list of patterns to apply to object names for inclusion. If
--- ommited, use `{'(.+).lua$'}`
...
---@return table Return a list of found objects or nil
function fs.find(params)
  local directory, matches, excludes, files, limit, type

  if params and is.a_string(params) then
    directory = params
  elseif params and is.a_array(params) then
    directory = params[1] or nil
    matches = params[2] or nil
    excludes = params[3] or nil
    type = params[4] or nil
    limit = params[5] or nil
  elseif params and is.a_dictionary(params) then
  ...
  if is.empty(directory) then
    ...
    directory = fs.parent(fullname)
    local f = fs.filename(fullname)
    local ex = f .. '$'
  ...
  Logger:trace("Finding all %ss in '%s' that match '%s' except '%s'", type, directory, matches, excludes)
  ...
  files = vim.fs.find(function(name, path)
    ...
    return result
  end, { type = type, limit = limit, path = directory })

  if #files == 0 then
    Logger:trace('No files found')
  else
    Logger:trace('- found %s', files)
  end
