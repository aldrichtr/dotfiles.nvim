...
local LspClient = class('LspClient')
...
function LspClient:disable_file_watching()
  ...
  if client_capabilities.workspace then
    client_capabilities.workspace.didChangeWatchedFiles = nil
  end
...
function LspClient:add_commands()
  ...
  cmd( {"LspAttach"}, {desc = "LSP actions",
    callback = function(event)
      set("n", "<F2>", function() buf.rename() end,
        { desc = "Renames all references to the symbol under the cursor",
          buffer = event.buf})
      set("n", "gL", function() lsp.inlay_hint.enable(not lsp.inlay_hint.is_enabled()) end,
        { desc = "Toggle inlay hints",
          buffer = event.buf})
      set("n", "gK", function() buf.hover() end,
          { desc = "Displays hover information",
            buffer = event.buf})
      set("n", "grd", function() buf.definition() end,
          { desc = "Jump to the definition",
            buffer = event.buf})
      set("n", "grD", function() buf.declaration() end,
        { desc = "Jump to declaration",
          buffer = event.buf})
      set("n", "gl", function() diag.open_float() end,
        { desc = "Show diagnostics in a floating window",
          buffer = event.buf})
      set("n", "gL", function() codelens.enable(not codelens.is_enabled()) end,
        { desc = "Toggle codelens",
          buffer = event.buf})
      set("n", "[d", function() diag.goto_prev() end,
        { desc = "Move to the previous diagnostic",
          buffer = event.buf})
      set("n", "]d", function() diag.goto_next() end,
